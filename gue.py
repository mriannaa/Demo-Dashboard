...
# Custom CSS - WARNA TEKS BERJALAN ADAPTIF & SIMETRIS
st.markdown("""
    <style>
    /* Background utama */
    ...
    div[data-testid="stMetricValue"] { 
        ...
        color: #6366f1 !important; /* Warna Indigo */
        width: 100% !important;
        text-align: center !important;
    ...
    /* Gaya Teks Berjalan Otomatis (Adaptif Light/Dark Mode) */
    .marquee-text {
        font-size: 14px !important;
        font-weight: 700 !important;   
        color: var(--text-color) !important; /* Mengikuti warna teks dasar tema aktif */
...
# PANEL PENGURUTAN DATA (SORTING) UNTUK TAB 2
st.sidebar.markdown("---")
st.sidebar.subheader("📊 Pengurutan Atribut Layanan")
sort_by = st.sidebar.selectbox(
    "Urutkan Aspek Berdasarkan:",
    ["Nama Aspek", "Skor Kepuasan Aktual", "Dampak terhadap Kepuasan"]
)
sort_order = st.sidebar.radio("Arah Pengurutan:", ["Kecil ke Besar (Ascending)", "Besar ke Kecil (Descending)"])

# PENGAMAN UTAMA: Deklarasi variabel is_ascending yang kokoh & anti-lewat
is_ascending = True
if "Besar ke Kecil" in sort_order:
    is_ascending = False
...
tab1, tab2 = st.tabs(["📈 Tren, Wilayah & Demografi", "📊 Analisis Atribut Layanan (Deep-Dive)"])
...
with tab1:
    ...
    with col_left:
        ...
        fig_trend = px.line(
            ...
            markers=True,
            color_discrete_sequence=["#5dfb4f", "#4850ec", "#F50B0B"]
        )
    ...
    with col_demo2:
        ...
        fig_gender = px.pie(df_gender, values='count', names='Gender', color_discrete_sequence=["#5ca9f6", '#ec4899'])
        st.plotly_chart(fig_gender, use_container_width=True)

with tab2:
    st.subheader("Analisis Driver Kepuasan Pasien (Korelasi vs Performa Aktual)")
    st.markdown("*Grafik memetakan aspek layanan mikro berdasarkan tingkat kepuasan dan nilai korelasi terhadap CSI.*")
    ...
    for tp in touchpoints:
        ...
        if len(valid_data) > 1:
            corr = valid_data[tp].corr(valid_data['CSI'])
        else:
            corr = 0
    ...
    # LOGIKA SORTING BERDASARKAN FILTER SIDEBAR (AMAN & STABIL)
    if sort_by == "Nama Aspek":
        df_driver = df_driver.sort_values(by='Aspek Layanan', ascending=is_ascending).reset_index(drop=True)
    elif sort_by == "Skor Kepuasan Aktual":
        df_driver = df_driver.sort_values(by='Skor Kepuasan Aktual', ascending=is_ascending).reset_index(drop=True)
    elif sort_by == "Dampak terhadap Kepuasan":
        df_driver = df_driver.sort_values(by='Dampak terhadap Kepuasan (Korelasi)', ascending=is_ascending).reset_index(drop=True)
        
    fig_driver = px.scatter(
        df_driver, x='Skor Kepuasan Aktual', y='Dampak terhadap Kepuasan (Korelasi)',
        text='Aspek Layanan', size=[12]*len(df_driver), color='Dampak terhadap Kepuasan (Korelasi)',
    ...
    st.markdown("**Tabel Ringkasan Parameter Layanan (Tersortir):**")
